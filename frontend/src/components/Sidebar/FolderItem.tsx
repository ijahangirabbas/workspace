...
function PageRow({
  ...
  const { setActivePageId, setActiveFolderId, updatePage, togglePinPage, deletePage } =
    useWorkspace();
  const [renaming, setRenaming] = useState(false);
  ...
  return (
    ...
    >
      ...
      <div className="page-row-actions">
        <button
          className={`icon-button small ghost ${page.isPinned ? "active" : ""}`}
          aria-label={page.isPinned ? "Remove from Favorites" : "Add to Favorites"}
          title={page.isPinned ? "Remove from Favorites" : "Add to Favorites"}
          onClick={(e) => {
            e.stopPropagation();
            void togglePinPage(page._id);
          }}
        >
          <Star
            size={13}
            className={`star-icon ${page.isPinned ? "filled" : ""}`}
          />
        </button>
        <button
          className="icon-button small ghost"
      ...
      <ContextMenu open={menuOpen} onClose={() => setMenuOpen(false)}>
        <button onClick={() => void togglePinPage(page._id)}>
          <Star size={14} className={page.isPinned ? "filled" : ""} />
          {page.isPinned ? "Unfavorite" : "Favorite"}
        </button>
        <button onClick={() => setRenaming(true)}>
          <Pencil size={14} />
          Rename
        ...
        <button
          className="danger"
          onClick={() => void deletePage(page._id)}
        >
          <Trash2 size={14} />
          Move to Trash
        </button>
