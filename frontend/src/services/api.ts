...
export const api = {
  ...
  getTrashFolders: () => request<Folder[]>("/folders/trash"),
  createFolder: (data: Partial<Folder>) =>
    request<Folder>("/folders", body("POST", data)),
  updateFolder: (id: string, data: Partial<Folder>) =>
    request<Folder>(`/folders/${id}`, body("PATCH", data)),
  deleteFolder: (id: string) =>
    request<void>(`/folders/${id}`, { method: "DELETE" }),
  restoreFolder: (id: string) =>
    request<Folder>(`/folders/${id}/restore`, { method: "POST" }),
  permanentDeleteFolder: (id: string) =>
    request<void>(`/folders/${id}/permanent`, { method: "DELETE" }),

  getPages: () => request<Page[]>("/pages"),
  getTrashPages: () => request<Page[]>("/pages/trash"),
  createPage: (data: Partial<Page>) =>
    request<Page>("/pages", body("POST", data)),
  updatePage: (id: string, data: Partial<Page>) =>
    request<Page>(`/pages/${id}`, body("PATCH", data)),
  ...
  permanentDeletePage: (id: string) =>
    request<void>(`/pages/${id}/permanent`, { method: "DELETE" }),
  emptyTrash: () =>
    request<void>("/pages/trash/empty", { method: "DELETE" }),
};
